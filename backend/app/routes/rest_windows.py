...
def create_rest_window():
    ...
    try:
        ...
        # idle 室禁止开窗（不改动 fruiting/idle/sanitize 状态机本身）
        if room.status == "idle":
            return jsonify({"detail": "idle 状态的出菇室禁止开设休整窗"}), 409

        # 同室时间窗相交则 409（半开区间 [start, end)，首尾相接不算相交）
        start = data["start_at"]
        end = data["end_at"]
        overlap = (
            db.query(RestWindow)
            .filter(
                RestWindow.room_id == room.id,
                RestWindow.start_at < end,
                RestWindow.end_at > start,
            )
            .first()
        )
        if overlap:
            return jsonify({"detail": "该出菇室在此时间段已存在相交的休整窗"}), 409
